...
def get_unlabeled_pretrain_data(args):
    ...
    X_train = torch.load(path + 'X_pretrain.pt')
...
def get_labeled_pretrain_data(args, version):
    path = args.path + args.dataset + '/' + args.task + '_data/'

    finetune_data_path = path + args.finetune_samples + '/'
    if args.finetune_samples == 'train':
        X_train = torch.load(finetune_data_path + 'X_' + args.finetune_samples + '.pt')
    else:
        X_train = torch.load(finetune_data_path + 'X_v' + str(version) + '.pt')
    X_val = torch.load(path + 'val/X_val.pt')
    print('X_train: ' + str(X_train.shape))
    print('X_val: ' + str(X_val.shape))
...
def get_finetune_data(args, version):
    path = args.path + args.dataset + '/' + args.task + '_data/'

    finetune_data_path = path + args.finetune_samples + '/'
    if args.finetune_samples == 'train':
        X_train, y_train = torch.load(finetune_data_path + 'X_' + args.finetune_samples + '.pt'), torch.load(finetune_data_path + 'y_' + args.finetune_samples + '.pt')
    else:
        X_train, y_train = torch.load(finetune_data_path + 'X_v' + str(version) + '.pt'), torch.load(finetune_data_path + 'y_v' + str(version) + '.pt')
    
    input_dim = (X_train.shape[2] - 1) // 2
    X_val, y_val = torch.load(path + 'val/X_val.pt'), torch.load(path + 'val/y_val.pt')
    X_test, y_test = torch.load(path + 'test/X_test.pt'), torch.load(path + 'test/y_test.pt')

    if args.dataset == 'PersonActivity' and args.task == 'classification' and not args.classify_pertp:
        y_train = torch.argmax(y_train[ : , 0, :], axis = 1)
        y_val = torch.argmax(y_val[ : , 0, :], axis = 1)
        y_test = torch.argmax(y_test[ : , 0, :], axis = 1)

    print('X_train: ' + str(X_train.shape) + ' y_train: ' + str(y_train.shape))
    print('X_val: ' + str(X_val.shape) + ' y_val: ' + str(y_val.shape))
    print('X_test: ' + str(X_test.shape) + ' y_test: ' + str(y_test.shape))

    if args.task == 'classification' and (args.dataset != 'PersonActivity' or (args.dataset == 'PersonActivity' and not args.classify_pertp)):
    ...
    elif args.task == 'classification' and args.dataset == 'PersonActivity' and args.classify_pertp:
        train_data_combined = TensorDataset(X_train, y_train.long())
        val_data_combined = TensorDataset(X_val, y_val.long())
        test_data_combined = TensorDataset(X_test, y_test.long())
